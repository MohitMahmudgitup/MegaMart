      ...
      const importData = async () => {
        try {
          ...
          if (importedData.name) {
            ...
            const newSlug = slugify(importedData.name, {
              lower: true,
              strict: true,
            });
          ...
          if (importedData.price) {
            form.setValue("productInfo.price", importedData.price, { shouldValidate: true });
          }
          if (importedData.salePrice) {
            form.setValue("productInfo.salePrice", importedData.salePrice, { shouldValidate: true });
          }

          // Set dimensions from package info
          if (importedData.packageInfo) {
            if (importedData.packageInfo.width) {
              form.setValue("productInfo.width", `${importedData.packageInfo.width}cm`, { shouldValidate: true });
            }
            if (importedData.packageInfo.height) {
              form.setValue("productInfo.height", `${importedData.packageInfo.height}cm`, { shouldValidate: true });
            }
            if (importedData.packageInfo.length) {
              form.setValue("productInfo.length", `${importedData.packageInfo.length}cm`, { shouldValidate: true });
            }
          }

          // Set video URL
          if (importedData.video && importedData.video !== '0') {
            form.setValue("video", importedData.video, { shouldValidate: true });
          }
