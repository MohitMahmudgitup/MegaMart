...
export default function ShopsPage() {
  ...
  const paginatedShops = useMemo(() => {
    ...
    return filteredShops.slice(startIdx, startIdx + ITEMS_PER_PAGE);
  }, [filteredShops, currentPage]);

  const mappedShops = paginatedShops.map((shop: any) => ({
    id: shop._id,
    name: shop.basicInfo?.name || "Unnamed Shop",
    tagline: shop.basicInfo?.slug || "",
    description: shop.basicInfo?.description || "",
    location: `${shop.shopAddress?.city || ""}, ${shop.shopAddress?.country || ""}`,
    created: shop.createdAt ? new Date(shop.createdAt).toLocaleDateString() : "",
    revenue: shop.currentBalance ? `$${shop.currentBalance.toLocaleString()}` : "$0",
